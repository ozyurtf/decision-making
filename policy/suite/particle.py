...
class RGBArrayAsObservationWrapper(dm_env.Environment):
    ...
    def __init__(self, env, width=84, height=84):
        ...
        # The environment given in the input provides an 
        # - observation space, 
        # - action space, 
        # - step function (takes an action and returns 
        # next state, reward, and whether episode is done),
        # - goal, 
        # - reset function (resets the environment to an initial state)
        self._env = env
        
        # Store the environment and rendering dimensions of the provided environment env
        self._width = width
        self._height = height
        
        # Reset the environment, initialize a state for the agent, 
        # initialize a goal (optional), reset the goal (optional),
        # initialize a 2D grid that shows the valid state space, 
        # regions where obstacles exist, and region where 
        # target position (goal) is located.
        self._env.reset()
        
        # Define a new observation space for the RGB image data 
        # and maintain the same action space as the env
        ...
        # Define the minimum and maximum action values 
        # that can be taken in the action space as 'minimum' and 'maximum' attributes.
        ...
        # Create an action space that is suitable for DeepMind's environment interface.
        self._action_spec = specs.BoundedArray(shape = wrapped_action_spec.shape,
                                               dtype = np.float32,
                                               minimum = wrapped_action_spec.minimum,
                                               maximum = wrapped_action_spec.maximum,
                                               name = 'action')
        
        # Create an observation space that is suitable for Deepmin'd environment interface.
        self._obs_spec = {}
